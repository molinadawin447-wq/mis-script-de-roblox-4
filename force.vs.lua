...
Net = Packages:WaitForChild("Net")
children = Net:GetChildren()
u1 = ReplicatedStorage.Packages
u2 = require(u1.Net:Clone())
ProxyUseItem = u2:RemoteEvent("UseItem")
StringedUseItem = tostring(ProxyUseItem)
RealUseItem = ReplicatedStorage.Packages.Net[StringedUseItem]
ProxyOnTeleport = u2:RemoteEvent("QuantumCloner/OnTeleport")
StringedOnTeleport = tostring(ProxyOnTeleport)
RealOnTeleport = ReplicatedStorage.Packages.Net[StringedOnTeleport]
local spamRemotes = {}
...
local ProxyAdmin = u2:RemoteFunction("a0e78691-cb9b-4efc-ac08-9c06fea70059")
local StringedAdmin = tostring(ProxyAdmin)
local adminRemote = ReplicatedStorage.Packages.Net[StringedAdmin]
...
local Defaults = {
	...
	ActionBtnTeleportNowX = 16,
	ActionBtnTeleportNowY = 300,
	ActionBtnAPSpamX = 16,
	...
	LineToBest = true,
	LineToBase = true,
	AutoStealPriorityEnabled = false,
...
local function triggerBestTeleport()
	...
	task.spawn(function()
		pcall(function()
			if _G.TeleportToHighest then
				_G.TeleportToHighest(true)
			end
		end)
...
addButton("Stealing", "TP To Best", function()
	triggerBestTeleport()
	notify("Teleporting...", C.purple, 2)
end)
...
local Keybinds = {
	...
	TpToBest = {
		key = G.KeybindTpToBest or "T",
		fn = function()
			triggerBestTeleport()
			notify("Teleporting to best...", C.purple, 2)
		end,
	},
	ToggleGui = {
...
addToggle("Settings", "Show TP To Best", "ShowBtnTpToBest")
...
addToggle("Settings", "Show Teleport Now", "ShowBtnTeleportNow")
...
addButton("Settings", "TP To Best", function()
	if Keybinds.TpToBest.fn then
		Keybinds.TpToBest.fn()
	end
end)
...
(function()
	...
	refreshActionButtonsLockButton()
	makeActionButton("TP TO BEST", "ActionBtnTpBestX", "ActionBtnTpBestY", "ShowBtnTpToBest", function()
		if Keybinds.TpToBest.fn then
			Keybinds.TpToBest.fn()
		end
	end, "ActionBtnTpBestKey")
	...
	makeActionButton("TELEPORT NOW", "ActionBtnTeleportNowX", "ActionBtnTeleportNowY", "ShowBtnTeleportNow", function()
		if _G.isTeleporting then
			return
		end
		_G.isTeleporting = true
	...
	task.spawn(function()
		...
		while task.wait(0.1) do
			local state = table.concat({
				...
				tostring(G.ShowBtnTpToBest ~= false),
				...
				tostring(G.ShowBtnTeleportNow ~= false),
...
;(function()
    local _sideTP_allAnimalsCache = {}
    local _sideTP_isTpMoving = false
    local _sideTP_startSideTP
    local _stp_LP = player
    local _stp_WS = Workspace
    local _stp_TOOL_NAME = "Flying Carpet"
    local _stp_myName = player.DisplayName

    _G._sideTP_allAnimalsCache = _sideTP_allAnimalsCache
    _G._sideTP_isTpMoving = false
    _G._sideTPScanning = true

    local function _stp_makeTargetFromBrainrot(br)
        if not br then
            return nil
        end
        local plot = typeof(br.plot) == "Instance" and br.plot or plots:FindFirstChild(br.plotName or "")
        local plotName = br.plotName or (plot and plot.Name)
        if not plotName then
            return nil
        end
        return {
            plot = plotName,
            slot = tostring(br.slot or ""),
            name = br.displayName or br.name,
            owner = plot and getPlotOwner(plot) or nil,
            genValue = br.num or br.genValue or 0,
            mutation = br.mutation,
            _brainrot = br,
        }
    end

    local function _stp_findCachedBrainrot(targetPetData)
        if not targetPetData then
            return nil
        end
        local direct = targetPetData._brainrot
        if direct and direct.prompt and direct.prompt.Parent then
            return direct
        end
        local targetPlot = targetPetData.plot
        local targetSlot = tostring(targetPetData.slot or "")
        for _, br in ipairs(cachedBrainrots) do
            local plotName = br.plotName or (typeof(br.plot) == "Instance" and br.plot.Name or nil)
            if plotName == targetPlot and tostring(br.slot or "") == targetSlot then
                return br
            end
        end
        return nil
    ...
    local function _stp_refreshBrainrotCache()
        for i = #_sideTP_allAnimalsCache, 1, -1 do
            _sideTP_allAnimalsCache[i] = nil
        end
        for _, br in ipairs(cachedBrainrots) do
            local entry = _stp_makeTargetFromBrainrot(br)
            if entry and entry.owner and entry.owner ~= _stp_myName and type(entry.genValue) == "number" and entry.genValue >= 1 then
                _sideTP_allAnimalsCache[#_sideTP_allAnimalsCache + 1] = entry
            end
        end
        table.sort(_sideTP_allAnimalsCache, function(a, b)
            return (a.genValue or 0) > (b.genValue or 0)
        end)
        return _sideTP_allAnimalsCache
    end

    local function _stp_findAdorneeGlobal(target)
        local brainrot = _stp_findCachedBrainrot(target)
        if brainrot then
            local base = brainrot.base
            if base then
                local spawnPart = base:FindFirstChild("Spawn")
                if spawnPart then return spawnPart end
                return (base:IsA("BasePart") and base) or base:FindFirstChildWhichIsA("BasePart", true) or base
            end
            local model = brainrot.model
            if model and model:IsA("Model") then
                return model.PrimaryPart or model:FindFirstChildWhichIsA("BasePart", true)
            end
            local prompt = brainrot.prompt
            if prompt and prompt.Parent then
        ...
        local plot = _stp_WS:FindFirstChild("Plots") and _stp_WS.Plots:FindFirstChild(target.plot)
    ...
    local _stp_numBases = 4
    local _stp_sideOffsetX = -137
    local _stp_firstLeft = Vector3.new(-341, 14, 200)
    local _stp_firstRight = Vector3.new(-341, 14, 241)
    local _stp_leftSpacing = -107
    local _stp_rightSpacing = -107
    local _stp_sideOffsetR = Vector3.new(-5.036865, -0.01, 5.19937)
    local _stp_sideOffsetL = Vector3.new(-5.603241, -0.01, -4.993957)
    local _stp_Bases = {}
    for i = 0, _stp_numBases - 1 do
        local lPos = Vector3.new(_stp_firstLeft.X, _stp_firstLeft.Y, _stp_firstLeft.Z + _stp_leftSpacing * i)
        local rPos = Vector3.new(_stp_firstRight.X, _stp_firstRight.Y, _stp_firstRight.Z + _stp_rightSpacing * i)
        local sidePos = rPos + Vector3.new(-_stp_sideOffsetR.X, _stp_sideOffsetR.Y, _stp_sideOffsetR.Z)
        local otherSidePos = lPos + Vector3.new(-_stp_sideOffsetL.X, _stp_sideOffsetL.Y, _stp_sideOffsetL.Z)
        table.insert(_stp_Bases, { Side = 1, SidePos = sidePos, OtherSidePos = otherSidePos })
    end
    for i = 0, _stp_numBases - 1 do
        local lPos = Vector3.new(_stp_firstLeft.X + _stp_sideOffsetX, _stp_firstLeft.Y, _stp_firstLeft.Z + _stp_leftSpacing * i)
        local rPos = Vector3.new(_stp_firstRight.X + _stp_sideOffsetX, _stp_firstRight.Y, _stp_firstRight.Z + _stp_rightSpacing * i)
        local sidePos = rPos + _stp_sideOffsetR
        local otherSidePos = lPos + _stp_sideOffsetL
        table.insert(_stp_Bases, { Side = 2, SidePos = sidePos, OtherSidePos = otherSidePos })
    end

    local function _stp_getClosestBaseSide(promptPos)
        local desiredSide = (promptPos.X > -410) and 1 or 2
        local bestSidePos, bestOtherSidePos = nil, nil
        local bestDist = math.huge
        for _, base in ipairs(_stp_Bases) do
            if base.Side == desiredSide then
                local d1 = math.abs(promptPos.Z - base.SidePos.Z)
                local d2 = math.abs(promptPos.Z - base.OtherSidePos.Z)
                if d1 < bestDist then
                    bestDist = d1
        ...
        return bestSidePos, bestOtherSidePos
    end

    local _stp_clampMax = Vector3.new(-411.98956298828125, -5.408802032470703, 169.7320556640625)
    local _stp_clampMin = Vector3.new(-408.99603271484375, -5.4088029861450195, -129.7340545654297)
    local _stp_POS1_Y = 50.5

    local function _stp_computePos1FromPos2(pos2, finalLookDir)
        local a = _stp_clampMin
        local b = _stp_clampMax
        local ab = b - a
        ...
        if (t2 - t1) * abDotLook > 0 then t2 = t1 end
        local finalPos1 = a + ab * t2
        return Vector3.new(finalPos1.X, _stp_POS1_Y, finalPos1.Z)
    end

    local function _stp_findPromptForTarget(targetPetData)
        if not targetPetData or not targetPetData.plot or not targetPetData.slot then return nil end
        local brainrot = _stp_findCachedBrainrot(targetPetData)
        if brainrot and brainrot.prompt and brainrot.prompt.Parent then
            return brainrot.prompt
        end
        local plotsFolder = _stp_WS:FindFirstChild("Plots")
        if not plotsFolder then return nil end
        local plot = plotsFolder:FindFirstChild(targetPetData.plot)
        if not plot then return nil end
        local podiums = plot:FindFirstChild("AnimalPodiums")
        if not podiums then return nil end
        local podium = podiums:FindFirstChild(targetPetData.slot)
        if not podium then return nil end
        local base = podium:FindFirstChild("Base")
    ...
    local function _stp_getPromptPosition(prompt)
        if not prompt or not prompt.Parent then return nil end
    ...
    local function _stp_computeCarpetPoints(targetPetData)
        local prompt = _stp_findPromptForTarget(targetPetData)
        if not prompt then return nil end
        local promptPos = _stp_getPromptPosition(prompt)
        if not promptPos then return nil end
        local char = _stp_LP.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if not hrp then return nil end
        local sidePos, otherSidePos = _stp_getClosestBaseSide(promptPos)
        if not sidePos or not otherSidePos then return nil end
        local pos2 = sidePos
        ...
        if not isFirstFloor and not isLower then
            ...
            local function distOutOfBounds(z)
                if z > _stp_clampMax.Z then return z - _stp_clampMax.Z
                elseif z < _stp_clampMin.Z then return _stp_clampMin.Z - z
                else return 0 end
            end
        ...
        local pos1 = _stp_computePos1FromPos2(pos2, finalLookDir)
        if isLower then pos2 = pos2 - Vector3.new(0, 15, 0) end
        return {
            pos1 = pos1,
            pos2 = pos2,
            finalLookDir = finalLookDir,
            brainrotPos = promptPos,
            origSidePos = sidePos,
            otherSidePos = otherSidePos,
        }
    end

    local function _stp_instantClone()
        if _G.isCloning then return end
        _G.isCloning = true
        pcall(function()
            local character = _stp_LP.Character
            if not character then return end
            local humanoid = character:FindFirstChildOfClass("Humanoid")
            if not humanoid then return end
            local quantumCloner = _stp_LP.Backpack:FindFirstChild("Quantum Cloner") or character:FindFirstChild("Quantum Cloner")
            if not quantumCloner then
                local _qcDeadline = os.clock() + 0.25
                while os.clock() < _qcDeadline do
                    quantumCloner = _stp_LP.Backpack:FindFirstChild("Quantum Cloner") or character:FindFirstChild("Quantum Cloner")
                    if quantumCloner then break end
                    game:GetService("RunService").Heartbeat:Wait()
                end
            end
            if not quantumCloner then return end
            _G.SuppressQuantumClonerEquipUntil = os.clock() + 1
            if quantumCloner.Parent ~= character then
                pcall(function() humanoid:EquipTool(quantumCloner) end)
                local _equipDeadline = os.clock() + 0.2
                while quantumCloner.Parent ~= character and os.clock() < _equipDeadline do
                    game:GetService("RunService").Heartbeat:Wait()
                end
            end
            local remote = _G.GetQuantumOnTeleportRemote and _G.GetQuantumOnTeleportRemote()
            if not remote then return end
            local cloneName = tostring(_stp_LP.UserId) .. "_Clone"
            local oldClone = workspace:FindFirstChild(cloneName)
            pcall(function() quantumCloner:Activate() end)
            local cloneInst
            for _ = 1, 120 do
                task.wait()
                local found = workspace:FindFirstChild(cloneName)
                if found and found ~= oldClone then cloneInst = found; break end
            end
            if not cloneInst then return end
            pcall(function() remote:FireServer() end)
        end)
        _G.isCloning = false
    end

    _sideTP_startSideTP = function()
        if _sideTP_isTpMoving then return end
        if not _G._stp_isManualTP and not _G.autoTPButtonEnabled then return end
        if _G.AntiDieEnable then
            pcall(_G.AntiDieEnable, "sidetp")
        end
        local overrideEntry = _G._stp_overrideTarget
        _G._stp_overrideTarget = nil
        local targetPetData = nil
        local bestEntry = overrideEntry

        if not bestEntry then
            _stp_refreshBrainrotCache()
        end
        if not bestEntry and _G._uiTopTarget and _G._uiTopTarget.owner ~= _stp_myName then
            local ut = _G._uiTopTarget
            for _, a in ipairs(_sideTP_allAnimalsCache) do
                if a and a.plot == ut.plot and tostring(a.slot) == tostring(ut.slot) and a.owner ~= _stp_myName then
                    bestEntry = a
                    break
                end
            end
            if not bestEntry then
                bestEntry = {
                    plot = ut.plot,
                    slot = tostring(ut.slot),
                    name = ut.name,
                    owner = ut.owner,
                    genValue = ut.genValue,
                    mutation = ut.mutation,
                }
            end
        end

        if not bestEntry and #_sideTP_allAnimalsCache > 0 then
            local _minVal = (_G.TPMinValue or 0) * 1e6
            local _prio = _G.PRIORITY_LIST or _G.SHARED_PRIORITY_ITEMS or {}
            for _, pName in ipairs(_prio) do
                local target = tostring(pName or ""):lower()
                if target ~= "" then
                    local bestForName, bestVal = nil, -math.huge
                    for _, a in ipairs(_sideTP_allAnimalsCache) do
                        if a and a.owner ~= _stp_myName and a.name
                            and tostring(a.name):lower() == target
                            and type(a.genValue) == "number" and a.genValue >= _minVal
                            and a.genValue > bestVal then
                            bestForName = a
                            bestVal = a.genValue
                        end
                    end
                    if bestForName then
                        bestEntry = bestForName
                        break
                    end
                end
            end
            if not bestEntry then
                local _bestVal = -math.huge
                for _, a in ipairs(_sideTP_allAnimalsCache) do
                    if a and a.owner ~= _stp_myName and type(a.genValue) == "number"
                        and a.genValue >= _minVal and a.genValue > _bestVal then
                        bestEntry = a
                        _bestVal = a.genValue
                    end
                end
        ...
        if bestEntry and not bestEntry._brainrot then
            local resolvedBrainrot = _stp_findCachedBrainrot(bestEntry)
            if resolvedBrainrot then
                bestEntry = _stp_makeTargetFromBrainrot(resolvedBrainrot) or bestEntry
            end
        end
        if bestEntry then targetPetData = bestEntry end
        if not targetPetData then return end

        pcall(function()
            _G.InstantPickupTarget = {
                plot = targetPetData.plot,
                slot = tostring(targetPetData.slot),
                name = targetPetData.name,
                owner = targetPetData.owner,
            }
            _G.InstantPickupTargetName = targetPetData.name
        end)
        ...
        _sideTP_isTpMoving = true
        _G._sideTP_isTpMoving = true

        pcall(function()
            if _G.GetQuantumOnTeleportRemote then _G.GetQuantumOnTeleportRemote() end
        end)

        local _sideCleanupList = {}
        local function finishTeleport()
            for i = #_sideCleanupList, 1, -1 do
                pcall(_sideCleanupList[i])
            end
            _sideCleanupList = {}
            _sideTP_isTpMoving = false
            _G._sideTP_isTpMoving = false
            _G._firstTPDone = true
        end

        local _carpet = _stp_computeCarpetPoints(targetPetData)
        if not _carpet then finishTeleport(); return end
        local pos1 = _carpet.pos1
        local pos2 = _carpet.pos2
        local finalLookDir = _carpet.finalLookDir
        local brainrotPos = _carpet.brainrotPos
        local origSidePos = _carpet.origSidePos
        local otherSidePos = _carpet.otherSidePos

        do
            local _slotNum = tonumber(targetPetData and targetPetData.slot) or 0
            local _isStairSideFloor1 = (_slotNum >= 6 and _slotNum <= 10 and brainrotPos.Y <= 8.9)
            local _baseX, _shift
            if _isStairSideFloor1 and otherSidePos then
                _baseX = otherSidePos.X
                _shift = 1
            else
                local _petPart = _stp_findAdorneeGlobal(targetPetData)
                _baseX = (_petPart and _petPart.Position and _petPart.Position.X) or pos2.X
                _shift = 2
            end
            local _landingX
            if _baseX > -410 then _landingX = _baseX + _shift else _landingX = _baseX - _shift end
            pos2 = Vector3.new(_landingX, pos2.Y, pos2.Z)
        end

        if brainrotPos.Y > 25 then
            pos2 = Vector3.new(pos2.X, 16, pos2.Z)
        elseif brainrotPos.Y > 10 then
            pos2 = Vector3.new(pos2.X, 5, pos2.Z)
        end

        if origSidePos and otherSidePos then
            local distNear = math.abs(pos2.Z - origSidePos.Z)
            local distFar = math.abs(pos2.Z - otherSidePos.Z)
            if distFar < distNear then
                pos1 = Vector3.new(pos1.X, pos1.Y, pos1.Z - 2)
            else
                pos1 = Vector3.new(pos1.X, pos1.Y, pos1.Z + 2)
            end
        else
        ...
        pcall(function()
            ...
            if _h and _h.Health > 0 then _h.Health = _h.MaxHealth end
        end)
        if _G.AntiDieEnable then
            _G.AntiDieEnable("sidetp")
        end
        table.insert(_sideCleanupList, function()
            if _G.AntiDieDisable then
                _G.AntiDieDisable("sidetp")
            end
        end)

        local carpet = _stp_LP.Backpack:FindFirstChild(_stp_TOOL_NAME) or char:FindFirstChild(_stp_TOOL_NAME)
        if carpet and hum and hum.Parent and carpet.Parent ~= char then
        ...
        pcall(function()
            local cam = _stp_WS.CurrentCamera
            if cam then
                local finalFacing = CFrame.lookAt(pos2, pos2 + finalLookDir)
                local camPos = pos2 - (finalFacing.LookVector * 15) + Vector3.new(0, 7, 0)
                cam.CFrame = CFrame.lookAt(camPos, pos2 + (finalFacing.LookVector * 5))
            end
        end)
        ...
        hrp.AssemblyLinearVelocity = Vector3.new(0, (_G.TPVelocity or 480), 0)
        task.wait(0.09)
        hrp.AssemblyLinearVelocity = Vector3.new(0, 10, 0)

        if not char.Parent or not hrp.Parent then finishTeleport(); return end
        local midPoint = Vector3.new((pos1.X + pos2.X) / 2, pos2.Y + 8, (pos1.Z + pos2.Z) / 2)
        hrp.CFrame = CFrame.new(midPoint, midPoint + finalLookDir)
        hrp.AssemblyLinearVelocity = Vector3.new(0, 10, 0)
        task.wait(0.055)
        ...
        hrp.AssemblyLinearVelocity = Vector3.new(0, 10, 0)
        task.wait(_G.LandingDelay or 0.48)
        if brainrotPos.Y > 18 then
            _G.LiftPlateTargetY = brainrotPos.Y
            _G.LiftPlateTargetX = brainrotPos.X
            _G.LiftPlateTargetZ = brainrotPos.Z
        end

        _G._stp_isManualTP = nil
        if not _G.AutoCloneAfterTP then finishTeleport(); return end

        local posBeforeClone = _stp_LP.Character and _stp_LP.Character:FindFirstChild("HumanoidRootPart") and _stp_LP.Character.HumanoidRootPart.Position or hrp.Position
        _stp_instantClone()
        while _G.isCloning do
            task.wait()
        end

        local newChar, newHRP
        local cloneSucceeded = false
        local cloneTimeout = os.clock() + 3
        while os.clock() < cloneTimeout do
            newChar = _stp_LP.Character
            if newChar then
        ...
        if not newChar then newChar = _stp_LP.CharacterAdded:Wait() end
        newHRP = newChar and newChar:WaitForChild("HumanoidRootPart", 3)
        if not cloneSucceeded or not newHRP then finishTeleport(); return end

        local _plotRad = (brainrotPos.Y <= 8.9) and 25.5 or 25
        ...
        if plotsFolder then
            ...
            for _, plot in ipairs(plotsFolder:GetChildren()) do
                pcall(function()
                    ...
                    if math.abs(pos.X - plotPos.X) < _plotRad and math.abs(pos.Z - plotPos.Z) < _plotRad then
                        inPlotRadius = true
                    end
                end)
        ...
        if not inPlotRadius then finishTeleport(); return end

        local _settleStart = os.clock()
        local _lastPos = newChar and newChar:FindFirstChild("HumanoidRootPart") and newChar.HumanoidRootPart.Position
        local _stableFrames = 0
        while os.clock() - _settleStart < 0.4 do
            task.wait()
            local _hrp = newChar and newChar:FindFirstChild("HumanoidRootPart")
            if _hrp and _lastPos then
                if (_hrp.Position - _lastPos).Magnitude < 0.05 then
                    _stableFrames = _stableFrames + 1
                    if _stableFrames >= 3 then break end
                else
                    _stableFrames = 0
                end
                _lastPos = _hrp.Position
            end
        end

        local _backpack = _stp_LP:FindFirstChild("Backpack")
        local newHum = newChar:FindFirstChildOfClass("Humanoid")
        newHRP = newChar and newChar:FindFirstChild("HumanoidRootPart")
        if not newHRP then finishTeleport(); return end
        _G._stp_lastTPBrainrotPos = brainrotPos
        _G._stp_lastTPBrainrotPlot = targetPetData and targetPetData.plot or nil
        local _isUpperFloor = (brainrotPos.Y > 10)

        if _isUpperFloor then
            pcall(function()
                if newHum then newHum.Health = newHum.MaxHealth end
            end)
            pcall(function()
                local bp = _stp_LP:FindFirstChild("Backpack")
                local c = bp and bp:FindFirstChild(_stp_TOOL_NAME)
                if c and newHum then newHum:EquipTool(c) end
            end)

            local itemPos = brainrotPos
            local verticalDiff = itemPos.Y - newHRP.Position.Y
            if itemPos.Y > 8 or verticalDiff > 2 then
                local airPos = Vector3.new(itemPos.X, newHRP.Position.Y, itemPos.Z)

                newHRP.AssemblyLinearVelocity = Vector3.new()
                newHRP.AssemblyAngularVelocity = Vector3.new()
                newHRP.CFrame = CFrame.new(airPos)
                finishTeleport()

                task.spawn(function()
                    local start = tick()
                    local releaseDownward = false
                    while tick() - start < 5 do
                        if not newHRP or not newHRP.Parent then break end
                        if _stp_LP:GetAttribute("Stealing") then
                            releaseDownward = true
                            break
                        end
                        newHRP.AssemblyLinearVelocity = Vector3.new(0, 38, 0)
                        task.wait(0.05)
                    end
                    if newHRP and newHRP.Parent then
                        ...
                        if releaseDownward then
                            newHRP.AssemblyLinearVelocity = Vector3.new(0, -28, 0)
                        else
                            newHRP.AssemblyLinearVelocity = Vector3.new()
                        end
                    end
            ...
            if _backpack and newHum then
                local c = _backpack:FindFirstChild(_stp_TOOL_NAME)
                if c then pcall(function() newHum:EquipTool(c) end) end
            end
            task.wait(0.02)
            newHRP = newChar and newChar:FindFirstChild("HumanoidRootPart")
            if newHRP and _G.TPDirectlyToPet then
                local tpPos = Vector3.new(brainrotPos.X, newHRP.Position.Y, brainrotPos.Z)
                newHRP.CFrame = CFrame.new(tpPos, tpPos + finalLookDir)
            end
            finishTeleport()
        end
    end

    _G._stp_startSideTP = _sideTP_startSideTP
end)()
task.spawn(function()
    task.wait(15)
    _G._firstTPDone = true
...
;(function()
    if G.autoTPButtonEnabled == nil then
        G.autoTPButtonEnabled = true
    end
    pcall(function()
    ...
    local function mapBrainrotTarget(br)
        if not br then
            return nil
        end
        ...
        if typeof(plot) ~= "Instance" then
            plot = plots:FindFirstChild(br.plotName or "")
        end

        local owner = plot and getPlotOwner(plot)
        return {
            plot = br.plotName or (plot and plot.Name),
            slot = tostring(br.slot or ""),
            name = br.displayName or br.name,
            owner = owner,
            genValue = br.num or br.genValue,
            mutation = br.mutation,
        }
    end

    local function syncTeleportGlobals(isManualTP)
        _G.autoTPButtonEnabled = G.autoTPButtonEnabled ~= false
        _G.TPDirectlyToPet = G.TPDirectlyToPet ~= false
        _G.PRIORITY_LIST = (G.AutoStealPriorityEnabled and G.PriorityNames and #G.PriorityNames > 0) and G.PriorityNames or nil
        _G._uiTopTarget = mapBrainrotTarget(selectedBrainrot)
        _G._stp_isManualTP = isManualTP and true or nil

        if _G.AutoCloneAfterTP == nil then
            _G.AutoCloneAfterTP = true
        end

        if type(_G.GetQuantumOnTeleportRemote) ~= "function" then
            _G.GetQuantumOnTeleportRemote = function()
                return RealOnTeleport
            end
        end
    end

    function runAutoSnipe(isManualTP)
        if tpState.moving then
            return
        end
        if type(_G._stp_startSideTP) ~= "function" then
            return
        end

        syncTeleportGlobals(isManualTP == true)
        tpState.moving = true
        ...
        tpState.moving = false
        if not ok then
            warn("runAutoSnipe failed:", err)
        end
    end

    _G.runAutoSnipe = runAutoSnipe
    _G.TeleportToHighest = function(isManualTP)
        if not isManualTP and tpState.spawned then
            _G.isTeleporting = false
            return
        end
        if tpState.moving then
        ...
        _G.isTeleporting = true
        if not isManualTP then
            tpState.spawned = true
        end

        task.spawn(function()
            local ok, err = pcall(function()
                runAutoSnipe(isManualTP)
            end)
            if not ok then
                warn("TeleportToHighest failed:", err)
            end
    ...
    task.spawn(function()
        local function doTP()
            if _G.isTeleporting or not G.autoTPButtonEnabled then
                return
            end

            _G.isTeleporting = true
            pcall(function()
                _G.TeleportToHighest()
            end)
            task.delay(1.5, function()
                _G.isTeleporting = false
            end)
        ...
        local function canBootstrapAutoTP()
            if not G.autoTPButtonEnabled then
                return false
            end
            if type(_G._stp_startSideTP) ~= "function" then
                return false
            ...
            return #cachedBrainrots > 0
        end

        local function bootstrapAutoTP()
            for _ = 1, 12 do
                if canBootstrapAutoTP() then
                    doTP()
                    return
                end
                task.wait(0.5)
    ...
    UserInputService.InputBegan:Connect(function(inp, gp)
        ...
        if inp.KeyCode.Name == (G.TPKeybind or "T") then
            if _G.isTeleporting then
                return
            end
    ...
    addToggle("Teleport", "Auto TP", "autoTPButtonEnabled")
    addToggle("Teleport", "Priority Mode", "AutoStealPriorityEnabled")
    addDivider("Teleport")
    addButton("Teleport", "Teleport Now", function()
        if _G.isTeleporting then
            return
        end
        ...
        notify("Teleporting...", C.purple, 2)
    end, C.purple, C.purpleHi, C.purpleDim)
    addKeybind("Teleport", "TP Keybind", G.TPKeybind or "T", function(k)
        G.TPKeybind = k
        saveConfig()
    end)
end)()
	...
	if key == "HoldDuration" then
		...
		if ok and isPrompt then
			...
			if actionOk and actionText == "Steal" then
				return pcall(old_newindex, self, key, value)
			end
