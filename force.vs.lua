...
local startDesyncSession = function() end
local stopDesyncSession  = function() end
...
for _, name in pairs({"AÃ˜NIXGUI"}) do
...
local PW = 220 -- portrait panel width
local CORNER = 12
...
portImg.Image = "rbxassetid://92081139728958"
...
saveConfig = function(btn)
	...
	if btn then
		local prev = btn.Text
		btn.Text = ok and "âœ“  Saved!" or "âœ•  Failed!"
...
local Presets={}
local PRESET_FILE="AÃ˜NIXMobilePresets.json"; local LAST_PRESET_FILE="AÃ˜NIXMobileLastPreset.json"
...
local function setupChar(char)
    ...
    if head then
        ...
        discordLbl.BackgroundTransparency=1; discordLbl.Text="/Ã˜NIX HUB"
...
local function playIntroAnimation()
	...
	local SOUL_LOGO_ASSET_ID = "rbxassetid://92081139728958"
	...
	task.spawn(function()
		pcall(function()
			...
			writefile("OnixHubIntro", game:HttpGet(RandomIntro))
			local flex1 = Instance.new("Sound", gethui())
			flex1.SoundId = getcustomasset("OnixHubIntro")
